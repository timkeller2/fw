...
#			City Processing
		...
		for i in range (CyMap().numPlots()):
			...
			for ii in range(pPlot.getNumUnits()):
				...
				if (bCanCreateUnit and CyGame().getSorenRandNum(300, "NewAnimal") < pUnit.getLevel() - 5 + pUnit.getFortifyTurns() and not pUnit.isHasPromotion(gc.getInfoTypeForString('PROMOTION_FATIGUED')) and (pUnit.getUnitCombatType() == gc.getInfoTypeForString('UNITCOMBAT_ANIMAL'))):
					pUnit.setHasPromotion(gc.getInfoTypeForString('PROMOTION_FATIGUED'), True)
